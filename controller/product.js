...
// PRODUCT BY ID //add review when getProductById
exports.getProductById = async (req, res) => {
  try {
    const { productId } = req.params;
    // const product = await Product.findById(productId).populate('reviews').populate('ratings')
    ...
    // .populate({
    //   path:"reviews",
    //   select:"productId userId reviewText",

    // })

    // ---debug---
    // const ratings = await Rating.find({product:productId})
    // const ratingName = ratings.map((rating)=>{
...
exports.searchProduct = async (req, res) => {
  try {
    ...
    if (!productName) {
      return res
        .status(400)
        .json({ message: "Product name query is required" });
    }
